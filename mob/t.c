...
void moov_chara(character_t *chara, glb_t *global)
{
    ...
    if ((sfKeyboard_isKeyPressed(sfKeyLeft) || sfKeyboard_isKeyPressed(sfKeyQ))) {
            anim_left(chara);
            if (check_coll(global->map, global->actual_map, global->chara, 1) == 0)
                do_key(chara, 1);
    }
    if ((sfKeyboard_isKeyPressed(sfKeyRight)|| sfKeyboard_isKeyPressed(sfKeyD))) {
            anim_right(chara);
            if (check_coll(global->map, global->actual_map, global->chara, 2) == 0)
                do_key(chara, 2);
    }
    if ((sfKeyboard_isKeyPressed(sfKeyDown)|| sfKeyboard_isKeyPressed(sfKeyS))) {
            anim_down(chara);
            if (check_coll(global->map, global->actual_map, global->chara, 4) == 0)
                do_key(chara, 3);
    }
    if ((sfKeyboard_isKeyPressed(sfKeyUp)|| sfKeyboard_isKeyPressed(sfKeyZ))) {
            anim_up(chara);
            if (check_coll(global->map, global->actual_map, global->chara, 3) == 0)
                do_key(chara, 4);
    }
}
...
character_t *init_chara(void)
{
    ...
    scale.x = 1.5;
    scale.y = 1.5;
    chara->texture = sfTexture_createFromFile("texture/sprite/main_chara.png", NULL);
...
void attack(glb_t *global)
{
    ...
    if (sfMouse_isButtonPressed(sfMouseLeft) == sfTrue && global->b_att == -1) {
        ...
        sfSprite_setPosition(global->attack, get_pos(global->chara, global->attack));
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (distance <= 20 && global->b_att >= 0 && global->mob[i].atta_wolf >= 7) {
