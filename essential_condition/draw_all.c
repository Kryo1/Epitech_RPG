...
{
    if (global->drop == 0) {
        sfRenderWindow_drawSprite(global->wind.window, global->inventory_base.sprite, NULL);
    }
    if (global->drop == 1) {
        sfRenderWindow_drawSprite(global->wind.window, global->inventory_1.sprite, NULL);
    }
    if (global->drop == 2) {
        sfRenderWindow_drawSprite(global->wind.window, global->inventory_2.sprite, NULL);
    }
    if (global->drop == 3) {
        sfRenderWindow_drawSprite(global->wind.window, global->inventory_3.sprite, NULL);
    }
}

void draw_2(glb_t *global)
{
    sfRenderWindow_drawSprite(global->wind.window, global->hud_sort_feu.sprite, NULL);
    sfRenderWindow_drawSprite(global->wind.window, global->hud_sort_foudre.sprite, NULL);
    sfRenderWindow_drawSprite(global->wind.window, global->hud_sort_epee.sprite, NULL);
...
void draw_1(glb_t *global)
{
    ...
    if (global->drop == 0) {
        sfRenderWindow_drawSprite(global->wind.window, global->pnj.sprite, NULL);
        draw_text_pnj(global);
    } else if (global->drop == 1) {
        sfRenderWindow_drawSprite(global->wind.window, global->demon1.sprite, NULL);
        draw_textdemon(global);
    } else if (global->drop == 2) {
        sfRenderWindow_drawSprite(global->wind.window, global->demon2.sprite, NULL);
        draw_textdemon2(global);
    } else if (global->drop == 3) {
        sfRenderWindow_drawSprite(global->wind.window, global->demon3.sprite, NULL);
    ...
    for (int i = 0; i < 3; i++) {
        if (global->mob[i].health > 0 && global->drop == 2 && sfSprite_getPosition(global->chara->sprite).x > 3600) {
            sfRenderWindow_drawSprite(global->wind.window, global->mob[i].sprite, NULL);
    ...
    if (global->mob[0].health <= 0 && global->mob[1].health <= 0 && global->mob[2].health <= 0 && global->drop == 2) {
        sfSprite_setPosition(global->sceptre.sprite, pos(4500, 2800));
        sfRenderWindow_drawSprite(global->wind.window, global->sceptre.sprite, NULL);
    } else
